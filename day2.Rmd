...
vary_pars <- function(value, par, pool = "Cv", par_default = VSEMgetDefaults()$best, PAR = VSEMcreatePAR(1:1000)){
  ...
  return(mean(out[, which(colnames(out) == pool)], 1))
}
...
YourFun <- function(mt, index, PAR = VSEMcreatePAR(1:1000)) {
  params <- apply(mt, MARGIN = 1, vary_pars, par = index)
  
  return(params)
}

x <- morris(YourFun, factors = 6, r = 4, list(type = "oat", levels = 5, grid.jump = 3), index = 1:6)
